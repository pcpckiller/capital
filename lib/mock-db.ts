...
export async function getFeeBreakdownForUser(_userId: string): Promise<FeeRow[]> {
  const startNav = 5_000_000;
  const months = [
    '2025-03',
    '2025-04',
    '2025-05',
  ...
  const mgmtRateAnnual = 0.005;
  const mgmtMonthly = mgmtRateAnnual / 12; // ~0.04167%
  const perfRate = 0.2;

  const rows: FeeRow[] = [];
  let hwm = startNav; // high-water mark (post-fee NAV)
  let nav = startNav;
  for (let i = 0; i < months.length; i++) {
    const month = months[i];
    const navStart = nav;
    const gross = returns[i];
    const navBeforeFee = navStart * (1 + gross);
    const hwmBefore = hwm;
    const managementFee = navBeforeFee * mgmtMonthly;
    let performanceFee = 0;
    if (navBeforeFee > hwmBefore) {
      const profitAbove = navBeforeFee - hwmBefore;
      performanceFee = profitAbove * perfRate;
    }
    const navEnd = navBeforeFee - managementFee - performanceFee;
    const hwmAfter = Math.max(hwmBefore, navEnd);
    rows.push({
      month,
      navStart: Math.round(navStart),
      grossReturn: gross,
      navBeforeFee: Math.round(navBeforeFee),
      managementFee: Math.round(managementFee),
      performanceFee: Math.round(performanceFee),
      navEnd: Math.round(navEnd),
      hwmBefore: Math.round(hwmBefore),
      hwmAfter: Math.round(hwmAfter)
    });
    nav = navEnd;
    hwm = hwmAfter;
  }
